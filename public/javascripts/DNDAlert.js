...
class Context {
  ...
  INIT_PRIVATE_TO_NAME() {
    const privateKeys = Object.keys(this.CONTEXT_PRIVATE_PROPS);
    const privateProps = {};
  ...
  CONTEXT_PROVIDER_GET(key) {
    ...
    if (this.CONTEXT_PRIVATE_RESPONSE_LIST.includes(key)) {
      if (this._context[key] === undefined) return this.CONTEXT_DEFAULT_VALUES[key];
...
class DNDAlert extends Context {
  ...
  INIT() {
    ...
    const titleStatus = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.title);
  ...
  async DRAW() {
    const onOpen = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.onOpen);
    const autoCloseDuration = this.CONTEXT_PROVIDER_GET(
  ...
  PRE_INIT() {
    ...
    this.OPEN_ANIMATION_CLOSE_CONFLICT_TIME = this.CLASS_LIST.openAnimation.duration + 100;
  }
  ...
  createContentBox() {
    const opacity = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.opacity);
  ...
  createAlertTitle() {
    const title = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.title);
  ...
  createSvgElement() {
    const type = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.type);
    if (!type) return;
  ...
  createAlertMessage() {
    const message = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.message);
    const type = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.type);
    const html = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.html);
    const textAlign = this.CONTEXT_PROVIDER_GET(
  ...
  createHeader() {
    ...
    const closeIcon = this.CONTEXT_PROVIDER_GET(
  ...
  bagCreator() {
    const CONTEXT = this.CONTEXT_PROVIDER_GET_ALL();
  ...
  createButtonGroup() {
    const buttons = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.buttons);
    if (buttons.length === 0) return false;

    const buttonGroup = document.createElement('div');
    this.classAdder(buttonGroup, [this.CLASS_LIST.superClass.button_group]);
    const tempButtonRefArray = [];
  ...
  containerClickClose() {
    const closeBackgroundClick = this.CONTEXT_PROVIDER_GET(
    ...
    if (closeBackgroundClick) {
      this.CONTEXT_PROVIDER_GET(
        ...
        const openAnimationStatus = this.CONTEXT_PROVIDER_GET(
          this.CONTEXT_QUERY_NAME.openAnimationStatus,
        );
  ...
  contextBoxDrag() {
    ...
    const contextBox = this.CONTEXT_PROVIDER_GET(
      this.CONTEXT_QUERY_NAME.content_boxRef,
    );

    const header = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.headerRef);
    const draggable = this.CONTEXT_PROVIDER_GET(
    ...
    if (draggable) {
      ...
      header.addEventListener('mousedown', (e) => {
        ...
        const shiftX = e.clientX - contextBox.getBoundingClientRect().left;
        const shiftY = e.clientY - contextBox.getBoundingClientRect().top;
      ...
      const title = this.CONTEXT_PROVIDER_GET(
        this.CONTEXT_QUERY_NAME.alert_titleRef,
      );
  ...
  getBodyElement() {
    const portalElement = this.CONTEXT_PROVIDER_GET(
  ...
  errorOptionsLoader() {
     ...
     */
    const props = this.CONTEXT_PROVIDER_GET_ALL();
  ...
  sourceControl() {
    if (
      ...
      const API = fetch('https://registry.npmjs.org/dndalertjs');
      API.then((response) => {
  ...
  removeContainer() {
    const animation = this.CONTEXT_PROVIDER_GET(
  ...
  async runOnClose() {
    const onClose = this.CONTEXT_PROVIDER_GET(this.CONTEXT_QUERY_NAME.onClose);
    if (onClose && !this.IS_CLOSE) {
      this.IS_CLOSE = true;
      let BAG = this.bagCreator();
      const TIME_DIFF = new Date().getTime() - BAG.PROPERTIES.CREATED_TIME;
      const HOW_MANY_SECONDS = Math.floor(TIME_DIFF / 1000);
      BAG = {
        ...BAG,
        PROPERTIES: {
          ...BAG.PROPERTIES,
  ...
  removeListener() {
    ...
    const closeButtonStatus = this.CONTEXT_PROVIDER_GET(
      this.CONTEXT_QUERY_NAME.closeIcon,
    );
  ...
  setBodyOverflow(ENUM_VALUE) {
    if (Object.values(this.OVERFLOW_ENUM).includes(ENUM_VALUE)) {
      const portalOverflowHidden = this.CONTEXT_PROVIDER_GET(
